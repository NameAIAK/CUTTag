...
# perl configureHomer.pl -install hg38 -keepScript#不更新脚本
...
for (my $i=0;$i<@ARGV;$i++) {
	if ($ARGV[$i] eq '-list') {
		...
		$specificVersion = $ARGV[++$i];
		$dependentFlag = 0;
		print STDERR "\tSince a specific version is specified, no dependent packages will be loaded\n";
	} elsif ($ARGV[$i] eq '-all') {
		$allFlag = 1;
...
if ($listFlag || $updateFlag || $removeFlag || $installFlag) { 
	...
	if (scalar(keys %install) < 1 && $installFlag) {
		$installList{'SOFTWARE'}->{'homer'}=$specificVersion;
	}
	foreach(keys %install) {
		...
		if (exists($dupName{$pkg})) {
			...
			my $g = $dupName{$pkg}->{'g'};
			$installList{$g}->{$p}=$specificVersion;
		} elsif (exists($update->{'packages'}->{$pkg})) {
			my $g = $update->{'packages'}->{$pkg};
			$installList{$g}->{$pkg}=$specificVersion;
		} else {
	...
	if ($installFlag) {
		...
		if ($dependentFlag) {
			foreach(@depCheck) {
				...
				foreach(keys %{$installList{$g}}) {
					...
					my $v = $update->{$g}->{$pkg}->{'version'};
					my $vv = $update->{"ORGANISMS"}->{$org}->{'version'};
					$installList{'ORGANISMS'}->{$org} = $vv;
					print STDERR "\t\tInstalling/Updating Organism $org ($vv) for package $pkg\n";
				}
			}
		}	
		...
		print STDERR "\tPausing for 3 seconds (hit ctrl-C to stop)\n";
		`sleep 3`;
		foreach(@groups) {
			my $mode = $_;
			foreach(keys %{$installList{$mode}}) {
				my $package = $_;
				my $wantedVersion = $installList{$mode}->{$package};
				my $url = $update->{$mode}->{$package}->{'url'};
				if ($wantedVersion ne '') {
					$url =~ s/\.v\d.*\.zip//;
					$url .= "." . $wantedVersion . ".zip";
				}
				print STDERR "\tInstalling: $package\n";
				print STDERR "\t\tDownloading...\n";

				my $Downloadfile = basename($url);
				if (-e $Downloadfile ) {
					print "文件 $Downloadfile  已存在，跳过下载。\n";
				} else {
					print "文件 $Downloadfile  不存在，正在下载...\n";
					system("wget -O \"$Downloadfile \" \"$url\"") == 0
						or die "下载失败: $!\n";
				}
				print STDERR "\t\tUnzipping...\n";
				`unzip -o -d "$homeDir" "$Downloadfile"`;

				# `wget -O homer.package.zip "$url"`;
				# if (-z 'homer.package.zip') {
				# 	print STDERR "!!! Error: Could not find $url\n";
				# 	print STDERR "Consider checking http://homer.ucsd.edu/homer/data/ for old package version\n";

				# 	# need to indicate failure here
				# 	next;
				# }
				# print STDERR "\t\tUnzipping...\n";
				# `unzip -o -d "$homeDir" homer.package.zip`;
				...
				if ($wantedVersion ne '') {
					$config->{$mode}->{$package}->{'version'} = $wantedVersion;
					$config->{$mode}->{$package}->{'url'} = $url;
				}
...
sub printCMD {
	...
	print STDERR "\t\t-version <version> (install specific version of package, i.e. '-version v4.9')\n";
	print STDERR "\t\t\tOlder version numbers can be found here: http://homer.ucsd.edu/homer/data/\n";
	print STDERR "\t\t-nodep (do NOT include additional dependent packages during installation)\n";
...
sub compileSoftware {
	...
	my @files = (
		...
		"../update/updateMotifScanningTracks.pl"
